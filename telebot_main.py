...
def edit_cart(data: types.CallbackQuery):
    direct_data, from_data = data.data.split(";")
...
def get_cart(msg: types.Message):
    ...
    if not cart_list:
        keyboard.add(
            types.InlineKeyboardButton("Обновить корзину", callback_data="refresh_cart")
        )
        ...
        if prod_in_cart.catalogue_item == product:
            ...
            buttons.append(
                types.InlineKeyboardButton(
                    ...
                    callback_data=f"В корзине {prod_in_cart.quantity}шт;{from_data}",
                )
            )
            buttons.append(
                types.InlineKeyboardButton(
